...
import { ctx, nextLevel } from "./app.js";
import { FIRE_SPAWN_INTERVAL, FIRE_COUNT } from "./utils.js";
...
export let gameManager = {
    ...
    spawnFire: function() {
        if (this.currentLevel !== 1 || this.fires.length >= FIRE_COUNT) return;
        
        const now = Date.now();
        if (now - this.lastFireSpawn < FIRE_SPAWN_INTERVAL) return;
        
        const spawnPoints = mapManager.entities.fireSpawnPoints;
    ...
    updateGameStatus: function() {
        ...
        if (this.gameWon) {
            statusElement.textContent = `Уровень ${this.currentLevel + 1} пройден! Время: ${timeStr}`;
            statusElement.style.color = 'green';
        } else if (!this.player.isAlive) {
            statusElement.textContent = `Поражение на уровне ${this.currentLevel + 1}! Время: ${timeStr}`;
    ...
    draw: function() {
        ...
        this.enemies.forEach(enemy => {
            if (enemy.isAlive) enemy.draw(ctx);
        });
        
        this.fires.forEach(fire => {
            if (fire.isActive) fire.draw(ctx);
        });
    },
